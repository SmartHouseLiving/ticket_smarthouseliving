        ...
        <div class="flex-none border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur-sm">
            <div class="flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                <div class="flex min-w-0 items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-cyan-600/80">
                            {{ optional($header)->header_badge ?: 'Automation Support Portal' }}
                        </p>
                        <h1 class="truncate text-base font-semibold tracking-tight text-slate-900 sm:text-lg">
                            {{ optional($header)->header_title ?: 'Centro de Suporte Técnico' }}
                        </h1>
                    </div>
                </div>
                ...
                @auth
                    <div class="flex items-center gap-3">
                        <div
                            class="hidden items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 sm:flex">
                            <div
                        ...
                        <a href="{{ route('account.settings') }}"
                            ...
                            {{ optional($header)->account_button_label ?: 'Conta' }}
                        </a>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <button type="submit"
                                ...
                                {{ optional($header)->logout_button_label ?: 'Logout' }}
                            </button>
                        </form>
                    </div>
                @endauth
